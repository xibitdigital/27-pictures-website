...
path). `admin` has full access, including publish. `editor` can create
series and toons but only **manages series/toons they created themselves**
(`series.owner_id` / `toons.owner_id` — a grouped toon follows its series'
owner, not its own column) and is **capped at `draft`/`staging`** — a 403 on
any attempt to set `published`. Gates live in `src/roles.ts`, applied in
`POST/PATCH /toons` and `PUT /series`.
...
| PUT | `/series` | JWT | create (any account) / update (owner or admin) series metadata (plate size + slot list) |
...
| POST | `/toons` | JWT | create — 403 if an editor tries `status: "published"`, or picks a series they don't own |
| GET | `/toons/:id` | JWT | full record |
| PATCH | `/toons/:id` | JWT | metadata + series + visibility — 403 for an editor who isn't the (series') owner, or tries to publish |
