...
describe("hashedCss", () => {
  it("emits a content-hashed copy and rewrites the reference", () => {
    fs.writeFileSync(path.join(out, "index.html"), '<link rel="stylesheet" href="/styles.css" />');
    const { files, html } = run();
  ...
  it("removes the unhashed copy, so no mutable path is served as immutable", () => {
  ...
  it("replaces a leftover ?v= query rather than stacking on it", () => {
  ...
  it("changes the hash when the contents change, and only then", () => {
    ...
    const referenced = () => /styles\.[a-f0-9]{10}\.css/.exec(run().html("index.html"))?.[0];
    ...
    const first = referenced();
    expect(first).toBeTruthy();

    rebuild("body{color:#fff}");
    expect(referenced()).toBe(first);

    rebuild("body{color:#000}");
    expect(referenced()).not.toBe(first);
  });

  it("rewrites nested pages and relative hrefs too", () => {
  ...
  it("leaves unrelated css and query strings alone", () => {
    fs.writeFileSync(
